# Different implementation of libraries, to work around scipy.misc being outdated.
# import imageio
import cv2
import os
...
def imread_resize(path):
    ...
    img_orig = imageio.imread(path) # scipy.misc.imread(path)
    #img = scipy.misc.imresize(img_orig, (227, 227)).astype(np.float)
    #img = np.resize(img_orig, (227, 227)).astype(np.float)
    img = cv2.resize(img_orig, (277, 277), interpolation = cv2.INTER_AREA)
    cv2.imwrite('outImage.jpg', img)
    #img = resizeimage.resize('thumbnail', img_orig, [227, 227])
...
def load_net(data_path):
    if not os.path.isfile(data_path):
        parser.error("Network %s does not exist. (Did you forget to download it?)" % datapath)
